...
function Circle({ height = 'h-8 md:h-16', width = 'w-8 md:w-16', bgColor = 'bg-yellow-500', borderRadius = 'rounded-full' }: CircleProps) {
  return <div className={cn(height, width, borderRadius, bgColor)} />;
}

function Cylinder({ text, height = 'h-8 md:h-16', width = 'w-24 md:w-48', bgColor = 'bg-slate-100' }: CylinderProps) {
  return (
    <div className={cn('relative flex items-center justify-center rounded-full', height, width, bgColor)}>
      <WaveReveal
  ...
  return (
    <div
      className={cn(
        className,
        'duration-500',
        animationEnd ? 'animate-out fade-out slide-out-to-left-full' : 'animate-in fade-in slide-in-from-right-full'
      )}
    >
  ...
  return (
    <div
      className={cn(
        className,
        'duration-700',
        animationEnd ? 'animate-out fade-out slide-out-to-right-full' : 'animate-in fade-in slide-in-from-left-full'
      )}
    >
...
export default function SlackIntro({
  ...
  return (
    <div className={cn('flex flex-col items-center justify-center gap-1 overflow-hidden bg-purple-950 py-4 md:gap-3')}>
